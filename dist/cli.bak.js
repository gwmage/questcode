...
async function main() {
    ...
    try {
        ...
        let ia = await (0, ia_1.loadIA)(iaFilePath, url);
        console.log('✅ IA 파일 로드/생성 완료.');
        browser = await playwright_extra_1.chromium.launch({ headless: false });
        ...
        for (const scenario of scenarios) {
            ...
            for (let step = 0; step < 20; step++) { // Max 20 steps per scenario
                ...
                try {
                    ...
                    const agentPrompt = createReactiveAgentPrompt(currentUrl, interactiveElements, actionLog, testContext, currentScenario.instruction);
                    ...
                    if (result.decision === 'finish') {
                        ...
                        if (instruction.includes('로그인') || instruction.includes('login'))
                            isLoggedIn = true;
                        if (instruction.includes('로그아웃') || instruction.includes('logout'))
                            isLoggedIn = false;
...
async function generateQAReport(ia, actionLog, scenarios) {
    ...
    const visitedNodes = [];
    function collectVisitedNodes(node) {
...
async function discoverAndAddLinks(page, ia, currentNode) {
    try {
        ...
        for (const link of newLinks) {
            if ((0, ia_1.addNodeToIA)(ia, currentNode.url, { url: link, title: 'TBD', status: 'unvisited' })) {
                linksAdded++;
            }
        }
        if (linksAdded > 0) {
...
function createReactiveAgentPrompt(currentUrl, pageElements, previousActions, testContext, currentScenario) {
...
async function executeAction(page, action, testContext) {
...
async function robustNurieRequest(prompt, options = {}) {
    const { retries = 3, chatId } = options;
    for (let i = 0; i < retries; i++) {
        try {
            return await (0, ai_service_1.nurieRequest)(prompt, chatId);
        }
        catch (error) {
