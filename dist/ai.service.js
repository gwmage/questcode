    ...
    switch (model) {
        case 'gpt-4o':
            if (!process.env.OPENAI_API_KEY)
                throw new Error('OPENAI_API_KEY is not set for gpt-4o.');
            return gpt4oRequest(prompt, chatId);
        case 'claude-3-opus':
            if (!process.env.ANTHROPIC_API_KEY)
                throw new Error('ANTHROPIC_API_KEY is not set for claude-3-opus.');
            return claude3OpusRequest(prompt, chatId);
        case 'gemini-2.5-pro':
            if (!process.env.GOOGLE_API_KEY)
                throw new Error('GOOGLE_API_KEY is not set for gemini-2.5-pro.');
        ...
        default:
            ...
            if (!process.env.OPENAI_API_KEY)
                throw new Error('OPENAI_API_KEY is not set for default model gpt-4o.');
            return gpt4oRequest(prompt, chatId);
    }
}
